...
|---|---|---|
| `restore --staged` — étape 2 |même |not staged for commit |
| `reset --soft HEAD~1` — étape 3 |même |rien => to be commited |
| `reset --mixed HEAD~1` — étape 4 |même |rien |
| `reset --hard HEAD~1` — étape 5 |annule le dernier commit |rien |

Répondez aussi à ces questions :

1. Pourquoi fallait-il refaire `add` après le reset mixed, mais pas après le reset soft ?
Il a été deplannfier par le reset mixed
...
Car sans push pas synchro sur serveur
